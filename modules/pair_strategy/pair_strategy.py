...
def calculate_long_position(i, instru, shares, stoploss, target, logs, edgecase):
    ...
    if logs : print_logs.append(str("\nFor capital of Rs.") + str(round(shares*curr_day_price,0)))
    ...
    if stoploss_c > stoploss:
      if logs : print_logs.append(str("\nStop Loss Hit : ({0}% reverse direction)".format(stoploss*100) ))
      ...
      if logs : print_logs.append(str("\nIf ignored Stop Loss then Loss : Rs.") + str(round(ignored_loss*shares,2)))

    # Check Target Achieved Condition
    elif target_c > target:
      if logs : print_logs.append(str("\nTarget Hit : ({0}% positive direction)".format(target*100)))
    ...
    # Profit Loss Statement
      ...
      if logs : print_logs.append(str("\nIf ignored Target then Gain : Rs.") + str(round(ignored_gain*shares,2)))

    # No Stoploss and No Target hit
    else :
      if logs : print_logs.append(str("\nNeither Target nor StopLoss is Hit" ))
